...
export default function SpoolAssignPage({
    ...
    try {
      ...
      if (!spoolsRes.ok) {
        if (spoolsRes.status === 400) {
          setError('Spoolman не настроен. Пожалуйста, сначала настройте Spoolman в разделе Настройки.');
        } else {
          setError('Не удалось загрузить данные катушек');
        }
        setLoading(false);
        return;
      }
      ...
      if (!foundSpool) {
        setError(`Катушка #${id} не найдена в Spoolman`);
        setLoading(false);
        return;
      }
        ...
        for (const printer of printersData.printers || []) {
          ...
          for (let i = 0; i < extSpools.length; i++) {
            trayOptions.push({
              ...
              label: extSpools.length > 1 ? `Внешняя катушка ${i + 1}` : 'Внешняя катушка',
              printer: printer.name,
            });
          }
        }
  ...
  const handleAssign = async (tray: TrayOption) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Не удалось назначить катушку');
      ...
      toast.error(err instanceof Error ? err.message : 'Не удалось назначить');
      setAssigning(false);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="w-full max-w-2xl mx-auto py-6 px-3 sm:px-4 md:px-6">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h1 className="text-xl sm:text-2xl font-bold">Назначить катушку</h1>
        </div>
        ...
        {!loading && error && (
          <Card className="border-destructive">
            <CardContent className="pt-6">
              ...
              <Link href="/scan">
                <Button variant="outline">Перейти к сканированию</Button>
              </Link>
            </CardContent>
          </Card>
        )}
        ...
        {!loading && !error && spool && (
          <div className="space-y-6">
            ...
            {trays.length > 0 ? (
              <Card>
                <CardHeader>
                  <CardTitle>Выберите лоток</CardTitle>
                  <CardDescription>
                    Выберите, в какой лоток AMS назначить эту катушку
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <Command className="rounded-lg border">
                    <CommandInput placeholder="Поиск лотков..." />
                    <CommandList className="max-h-[300px]">
                      <CommandEmpty>Лотки не найдены.</CommandEmpty>
                      <CommandGroup>
                        {trays.map((tray) => (
                          <CommandItem
                            key={tray.id}
              ...
              <Card>
                <CardContent className="pt-6">
                  <p className="text-muted-foreground text-center">
                    Принтеры или лотки не найдены. Пожалуйста, сначала настройте Home Assistant и обнаружьте принтеры.
                  </p>
